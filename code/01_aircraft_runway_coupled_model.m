%% Aircraft-runway coupled finite-element model
% Repository version: v1.0.2
% The large finite-element runway structural input is intentionally not
% bundled in the public repository. Place it locally at:
%   data/confidential/Run_result_input.mat
% The script will stop with a clear message when that file is absent.
...
fem_input = fullfile(data_dir, 'confidential', 'Run_result_input.mat');
if exist(fem_input, 'file') ~= 2
    error([ ...
        'Large/restricted finite-element runway input is not included in the public archive.\n', ...
        'Expected local file: %s\n', ...
        'The lightweight CCI demonstration can be run with code/02_cci_calculation.m without this file.' ...
        ], fem_input);
end
Run_ans = load(fem_input);
result_filename = fullfile(output_dir, 'Air_Run_result_120_50_30_sub7_C0w0_Enwell_2.mat');
...
x0 = width_runway/2;
max_error = 1e-8;
Le = length_runway - z0 - 5;
tm = Le/ve;
nt = floor(tm/dt);

fprintf('预计算不平整度数据...\n');
shice = load(fullfile(data_dir, 'example_roughness_profile.mat'));
...
ichol_opts = struct('type', 'ict', 'droptol', 1e-2, 'diagcomp', 0.1);
try
    L_init = ichol(Kr, ichol_opts); 
catch
    warning('内存紧张，降低预处理器精度...');
...
fprintf('开始主循环计算...\n'); 
tic;
backNode_dof_base = (0:3:(FEM_Sys_dof-1))';
...
for batch_start = 1:block_size:nt-1
    idx_end = min(batch_start + block_size - 1, nt-1);
